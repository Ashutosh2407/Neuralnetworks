...
class LossEpochCallback(keras.callbacks.Callback):
...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        ...
        if type(self.input_tensor) != type((1,2)):
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        ...
        if not self.model.layers:
            self.model.add(keras.layers.Dense(num_nodes,name=name,activation=activation,trainable=trainable,input_shape=self.input_tensor))
        else:
            self.model.add(keras.layers.Dense(num_nodes,name=name,activation=activation,trainable=trainable))
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        if not (self.model.layers):
            layer = keras.layers.Conv2D(num_of_filters,kernel_size=kernel_size,padding = padding,
            strides = strides,activation = activation,name = name,trainable =trainable,input_shape=self.input_tensor)
            self.model.add(layer)
            return layer
        else:
            layer  = keras.layers.Conv2D(num_of_filters,kernel_size=kernel_size,padding = padding,
            strides = strides,trainable =trainable,name = name, activation = activation)
            self.model.add(layer)
            return layer
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        layer = keras.layers.MaxPooling2D(pool_size=pool_size,strides=strides,padding=padding,data_format=None)
        self.model.add(layer)
        return layer
        ...
        if layer_number != None:
            if not layer_number:
                ...
                if layer_number>0:
                    ...
                    w = self.model.layers[layer_number-1].get_weights()
                else:
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        if layer_number != None:
            if layer_number>0:
                self.model.layers[layer_number-1].set_weights([weights,self.model.layers[layer_number-1].get_weights()[1]])
            else:
                self.model.layers[layer_number].set_weights([weights,self.model.layers[layer_number].get_weights()[1]])
        else:
            for l in self.model.layers:
                if l.name == layer_name:
                    l.set_weights([weights,l.get_weights()[1]])
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        if layer_number != None:
            if layer_number>0:
                self.model.layers[layer_number-1].set_weights([self.model.layers[layer_number-1].get_weights()[0],biases])
            else:
                self.model.layers[layer_number].set_weights([self.model.layers[layer_number].get_weights()[0],biases])
        else:
            for l in self.model.layers:
                if l.name == layer_name:
                    l.set_weights([l.get_weights()[0],biases])
    ...
    def remove_last_layer(self):
        ...
        """
        t = self.model.layers[-1]
        self.model = keras.models.Sequential(self.model.layers[:-1])
        return t
    ...
    def load_a_model(self,model_name="",model_file_name=""):
        ...
        if model_name == "":
            ...
            if model_name == "VGG16":
                ...
                self.model = keras.models.Sequential(layers=vgg_model.layers)
    ...
    def set_optimizer(self,optimizer="SGD",learning_rate=0.01,momentum=0.0):
        ...
        """
        if optimizer=="SGD":
            self.optimizer = keras.optimizers.SGD(lr = learning_rate,momentum = momentum)
